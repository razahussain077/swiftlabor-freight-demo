...
export async function POST(request: Request) {
  try {
    ...
    const response = await client.responses.create({
      ...
      instructions:
        "You are SwiftLabor's Lead Intelligence Agent. Research B2B companies for sales teams. Use web search before making factual claims. Prefer first-party company sources and reputable professional sources. Never invent people, buying signals, URLs, job titles, or evidence. Distinguish observed evidence from inference. Score ICP fit and buying intent independently. A buying signal must have a concrete evidence trail. If a decision maker cannot be verified, return an empty string rather than guessing.",
      input: `Research this prospect deeply.\n\nCompany/domain: ${company}\nICP criteria: ${icp || "B2B company with a credible need for AI-powered lead research, qualification, buying-signal detection, and sales workflow automation."}\n\nReturn a concise but evidence-led qualification record. Look for company size, business model, growth/hiring, sales motion, technology/automation initiatives, relevant operational pain, recent announcements, and other credible indicators of an active need. Identify the most relevant executive or revenue leader only when verifiable.`,
      text: {
        format: {
          type: "json_schema",
    ...
    return NextResponse.json({ ...parsed, agent: "Scout" });
  } catch (error) {
