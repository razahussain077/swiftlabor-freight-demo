...
function normalizeRecord(result: unknown) {
  ...
  const modelSources = Array.isArray(record.sources) ? record.sources.filter((source: any) => source && isAllowedUrl(String(source.url ?? ""))).slice(0, 8).map((source: any) => ({ title: cleanText(source.title || "Web source", 180), url: String(source.url), type: cleanText(source.type || "web", 40) })) : [];
  record.sources = modelSources.filter((source, index, all) => all.findIndex((item) => item.url === source.url) === index);
...
function getOpenRouterApiKey() {
  return process.env.OPENROUTER_API_KEY || process.env.swift || process.env.SWIFT || process.env.openrouter;
}

function getOpenRouterModel() {
  const configured = process.env.OPENROUTER_MODEL?.trim();
  // Older deployment configuration pointed at a Kimi free slug that OpenRouter
  // may not expose to the account. Use OpenRouter's free router instead; it
  // automatically selects an available free model supporting the request.
  if (!configured || configured === "moonshotai/kimi-k2.6:free") return "openrouter/free";
  return configured;
}

function getProviderError(providerError: unknown) {
  const error = providerError as any;
  return { status: Number(error?.status ?? error?.code ?? 0), message: String(error?.error?.message ?? error?.message ?? ""), raw: String(error?.error?.metadata?.raw ?? "") };
}
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ ...record, agent: "Scout", provider: "openrouter", model: getOpenRouterModel() });
    } catch (providerError) {
      const { status, message, raw } = getProviderError(providerError);
      console.error("scout-openrouter-failed", { status, message, raw, model: getOpenRouterModel() });
      ...
      const error = message === "OPENROUTER_API_KEY_MISSING"
        ...
        : isUnauthorized
          ...
          : isRateLimited
            ? "The selected free OpenRouter model pool is temporarily rate-limited. Please try again shortly."
            : isModelError
