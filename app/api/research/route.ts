...
const schema = {
  ...
  properties: {
    ...
    fitScore: { type: "integer" }, intentScore: { type: "integer" }, priority: { type: "string" },
    recommendedAction: { type: "string" },
    decisionMaker: {
      type: "object", additionalProperties: false,
      properties: { name: { type: "string" }, title: { type: "string" }, linkedin: { type: "string" }, confidence: { type: "string" }, whyRelevant: { type: "string" } },
      required: ["name", "title", "linkedin", "confidence", "whyRelevant"]
    },
    decisionMakers: {
      type: "array", maxItems: 3,
      items: { type: "object", additionalProperties: false,
        properties: { name: { type: "string" }, title: { type: "string" }, linkedin: { type: "string" }, confidence: { type: "string" }, whyRelevant: { type: "string" }, evidence: { type: "string" } },
        required: ["name", "title", "linkedin", "confidence", "whyRelevant", "evidence"] }
    },
    signals: {
      type: "array", maxItems: 5,
      items: { type: "object", additionalProperties: false,
        properties: { signal: { type: "string" }, evidence: { type: "string" }, strength: { type: "string" }, sourceUrl: { type: "string" } },
    ...
    research: {
      type: "object", additionalProperties: false,
      properties: { searchesPerformed: { type: "integer" }, pagesReviewed: { type: "integer" }, evidenceBacked: { type: "boolean" }, confidence: { type: "string" }, notes: { type: "string" } },
...
function normalizeUrl(value: unknown) {
  ...
  try { const u = new URL(raw); return u.protocol === "https:" || u.protocol === "http:" ? raw : ""; }
  catch { return ""; }
}
...
function buildAnalysisPrompt(company: string, icp: string, evidence: any[]) {
  const compactEvidence = evidence.map((r, i) => ({ source: i + 1, query: r.query, title: r.title, url: r.url, content: r.content }));
  return `You are Scout, a senior B2B lead-intelligence analyst. Analyze ONLY the live web evidence supplied below and return one compact qualification record.
...
async function callOpenRouter(company: string, icp: string, evidence: any[]) {
  ...
  if (!key) { const e: any = new Error("OPENROUTER_API_KEY_MISSING"); e.status = 503; throw e; }
  ...
  try {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "You are a precise B2B lead research analyst. Output one valid JSON object only." },
    ...
    if (!record) { console.error("scout-openrouter-non-json", { model: payload?.model || "openrouter/free", preview: text.slice(0, 500) }); const e: any = new Error("SCOUT_OPENROUTER_INVALID_JSON"); e.status = 502; throw e; }
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      else if (message === "SCOUT_NO_WEB_RESULTS") error = "Scout could not find enough public web evidence for this company.";
      else if (message === "SCOUT_OPENROUTER_INVALID_JSON") error = "Scout's AI analyst returned an unusable response. Please retry.";
      else if (message === "SCOUT_OPENROUTER_EMPTY_OUTPUT") error = "Scout's AI analyst returned no result. Please retry.";
      else if (status === 429) error = "The research provider rate limit was reached. Please wait a moment and retry.";
    ...
    return NextResponse.json({ error: "Scout could not process this request." }, { status: 500 });
  }
}
