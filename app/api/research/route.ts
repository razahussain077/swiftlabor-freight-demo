...
export async function POST(request: Request) {
  try {
    ...
    try { const record = await runOpenRouter(company, icp); return NextResponse.json({ ...record, agent: "Scout", provider: "openrouter", model: getOpenRouterModel() }); }
    catch (providerError) {
      ...
      const combined = `${message} ${raw}`.toLowerCase(); const isRateLimited = status === 429 || combined.includes("rate-limited") || combined.includes("rate limit") || combined.includes("quota"); const isUnauthorized = status === 401 || combined.includes("invalid api key") || combined.includes("unauthorized"); const isModelError = status === 404 || (combined.includes("model") && (combined.includes("not found") || combined.includes("unavailable")));
      const error = message === "OPENROUTER_API_KEY_MISSING" ? "OpenRouter API key is not configured. Add the Vercel environment variable named swift (or OPENROUTER_API_KEY) and redeploy." : isUnauthorized ? "OpenRouter rejected the API key. Check the Vercel variable named swift, then redeploy." : isRateLimited ? "Free model providers are temporarily rate-limited. Scout retried automatically; please try again in a moment." : isModelError ? `The OpenRouter model is unavailable: ${getOpenRouterModel()}. Set OPENROUTER_MODEL to an available model.` : "Scout could not complete the research through OpenRouter. Check the OpenRouter model configuration and deployment logs.";
